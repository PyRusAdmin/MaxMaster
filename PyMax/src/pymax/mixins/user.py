...
class UserMixin(ClientProtocol):
    ...
    async def get_users(self, user_ids: list[int]) -> list[User]:
        ...
        :rtype: list[User]
        """
        logger.debug("get_users ids=%s", user_ids)
        ...
        if missing_ids:
            logger.debug("Fetching missing users: %s", missing_ids)
        ...
        logger.debug("get_users result_count=%d", len(ordered))
        return ordered
    ...
    async def get_user(self, user_id: int) -> User | None:
        ...
        :rtype: User | None
        """
        logger.debug("get_user id=%s", user_id)
    ...
    async def fetch_users(self, user_ids: list[int]) -> list[User]:
        ...
        :rtype: list[User]
        """
        logger.info("Fetching users count=%d", len(user_ids))
        ...
        logger.debug("Fetched users: %d", len(users))
        return users
    ...
    async def search_by_phone(self, phone: str) -> User:
        ...
        self._users[user.id] = user
        logger.debug("Found user by phone: %s", user)
        return user
